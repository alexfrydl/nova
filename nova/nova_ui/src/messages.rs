...
impl MessageQueue {
  ...
  pub(crate) fn add_subscription(&mut self, target: Entity, type_id: TypeId) {
    self
      .subscriptions
      .entry(type_id)
      .or_default()
  ...
  pub(crate) fn remove_subscription(&mut self, target: Entity, type_id: TypeId) {
...
pub(crate) fn borrow_mut(res: &Resources) -> WriteResource<MessageQueue> {
