...
impl Framebuffer {
  ...
  pub fn new(context: &Arc<Context>) -> Self {
    Self { context: context.clone(), render_pass: None, image: None, framebuffer: None }
  }

  /// Returns the size of the framebuffer.
  pub fn size(&self) -> Size<u32> {
    self.image.as_ref().map(AsRef::as_ref).map(Image::size).unwrap_or_default()
  }

  /// Returns the render pass assigned to the framebuffer, if any.
  pub fn pass(&self) -> Option<&RenderPass> {
    self.render_pass.as_ref().map(AsRef::as_ref)
  }

  /// Sets the render pass the framebuffer will be used for.
  pub fn set_render_pass(&mut self, render_pass: &Arc<RenderPass>) {
    match self.render_pass.as_ref() {
      Some(current) if Arc::ptr_eq(current, render_pass) => return,
      _ => {
  ...
  pub fn set_attachment(&mut self, image: &Arc<Image>) {
    match self.image.as_ref() {
      Some(current) if Arc::ptr_eq(current, image) => return,
  ...
  pub fn ensure_created(&mut self) -> Result<(), OutOfMemoryError> {
    ...
    let render_pass = self.render_pass.as_ref().expect("framebuffer requires a render pass");

    let image = self.image.as_ref().expect("framebuffer requires a backbuffer attachment");

    let size = image.size();

    let extent = gfx_hal::image::Extent { depth: 1, width: size.width, height: size.height };
  ...
  pub fn as_backend(&self) -> &backend::Framebuffer {
